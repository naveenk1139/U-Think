...
export default function JobDetailsModal({ job, isSaved, onToggleSave, onClose }: JobDetailsModalProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-card rounded-3xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl relative animate-slide-up">
        
        {/* Header */}
        <div className="relative bg-gradient-to-r from-slate-900 to-indigo-950 p-6 sm:p-8 text-white shrink-0">
          <button 
            onClick={onClose}
            className="absolute top-4 right-4 p-2 bg-card/10 hover:bg-card/20 rounded-full transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <div className="flex gap-4 sm:gap-6 items-start">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3 mt-4 text-xs font-semibold">
                 <span className="bg-card/20 px-3 py-1.5 rounded-lg flex items-center gap-1.5">
        ...
        {/* Scrollable Content */}
        <div className="overflow-y-auto p-6 sm:p-8 space-y-8 flex-1 bg-background">
          
          {/* AI Match Analysis Section */}
          {job.matchAnalysis && (
            <div className="bg-card p-5 rounded-2xl border border-emerald-100 shadow-sm shadow-black/5 dark:shadow-none shadow-black/5 dark:shadow-none flex flex-col md:flex-row gap-6 relative overflow-hidden">
              <div className="absolute top-0 left-0 w-1.5 h-full bg-emerald-500"></div>
              <div className="flex flex-col justify-center shrink-0">
                <span className="text-sm font-bold text-text-muted uppercase tracking-widest">AI Match Score</span>
          ...
          <div className="space-y-4">
             <h3 className="text-lg font-bold text-text-primary border-b border-border pb-2">About the Role</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="space-y-2">
               <h4 className="text-xs font-bold text-text-muted uppercase tracking-widest">Required Skills</h4>
               <div className="flex flex-wrap gap-2">
                 {job.skills && job.skills.length > 0 ? job.skills.map((skill, i) => (
                   <span key={i} className="bg-card border border-border text-text-primary text-xs px-2.5 py-1 rounded-md shadow-sm shadow-black/5 dark:shadow-none shadow-black/5 dark:shadow-none">
                     {skill}
                   </span>
            ...
            <div className="space-y-4">
               <div>
                 <h4 className="text-xs font-bold text-text-muted uppercase tracking-widest mb-1">Salary Details</h4>
               ...
               <div>
                 <h4 className="text-xs font-bold text-text-muted uppercase tracking-widest mb-1">Experience Level</h4>
        ...
        {/* Footer Actions */}
        <div className="p-6 bg-card border-t border-border flex flex-wrap gap-4 items-center justify-between shrink-0">
          <div className="text-xs font-semibold text-text-muted uppercase tracking-wider flex items-center gap-2">
            Provided By: <span className="bg-background-secondary text-text-primary px-2 py-1 rounded">{job.source}</span>
          </div>
          
          <div className="flex gap-3">
             <button onClick={onClose} className="px-5 py-2.5 rounded-xl text-sm font-bold text-text-secondary hover:bg-background-secondary transition-colors">
               Cancel
             </button>
             {onToggleSave && (
               <button 
                 onClick={(e) => onToggleSave(e, job)}
                 className={`px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-colors border ${isSaved ? 'bg-blue-50 text-primary-hover border-blue-200' : 'bg-card text-text-primary border-border hover:bg-background'}`}
               >
